...
const uploadDirectory = join(process.cwd(), "public", "uploads", "portfolio");
...
async function savePortfolioImage(file: File, filename: string, buffer: Buffer) {
...
export async function POST(request: Request) {
  if (!(await isAuthorized(request))) {
    return NextResponse.json(
      { message: "Anda harus login sebagai admin untuk upload gambar." },
      { status: 401 },
    );
  }
  ...
  try {
    ...
    if (!file.type.startsWith("image/") && !extension) {
      return NextResponse.json(
        {
          message:
            "File belum terbaca sebagai gambar. Pilih file gambar seperti SVG, JPG, PNG, WebP, AVIF, dan format gambar lain.",
        },
        { status: 400 },
      );
    }
    ...
    const url = await savePortfolioImage(file, filename, buffer);

    return NextResponse.json({
      url,
      message: "Gambar portfolio berhasil diupload.",
    });
  } catch {
    return NextResponse.json(
      { message: "Gambar gagal diupload. Coba pilih file lain." },
      { status: 400 },
    );
  }
}
